...
#remove columns with less than 80% of the data present
Project_Data_Cleansed2 <- Project_Data_Cleansed %>%
  select_if(function(x) sum(is.na(x)) < 1000) 
...
train_rfModel <- function(train,test,original_test) {
  ...
  mtryValues <- seq(1, ncol(train)-1, by = 1)
...
train_and_select_knnModel <- function(train,test,original_test) {
  ...
  knnGrid <-  expand.grid(k = seq(1, 25, 2))
...
for (balance_func in balance_functions) {
  ...
  ### NNet
  # Plain
  NN.models <- train_nnetModel(balanced_training, split.test, main.test.df)
  # boruta
  NN.boruta.models <- train_nnetModel(balanced_training, split.test, main.test.df)
  # information gain
  NN.infogain.models <- train_nnetModel(balanced_training, split.test, main.test.df)
  # PCA
  NN.pca.models <- train_nnetModel(balanced_training, split.test, main.test.df)
...
for (balance_func in balance_functions) {
  ...
  cat("Random Forest with ", balance_func, ":\n")
  print(RF.results[[balance_func]])
  
  cat("KNN with", balance_func, ":\n")
  print(KNN.results[[balance_func]])
  
  cat("Neural Net with", balance_func, ":\n")
  print(NN.results[[balance_func]])
}
