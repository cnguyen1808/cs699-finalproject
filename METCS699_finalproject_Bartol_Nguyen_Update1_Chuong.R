...
corr <- cor(main.train.df[,-ncol(main.train.df)])
...
main.train.df <- main.train.df[, -CorrColumnRemoved]
main.test.df <- main.test.df[, -CorrColumnRemoved]
head(main.train.df)
...
boruta.train.df <- main.train.df
boruta.train.df$Class <- factor(boruta.train.df$Class, 
                                levels = c("Y", "N")) # N -> Y
borutaResult <- Boruta(Class ~ ., data = boruta.train.df)
...
boruta.train.df <- boruta.train.df[,!(names(boruta.train.df) %in% BorutaRejectedColumns)]
colnames(boruta.train.df)
boruta.test.df <- main.test.df[,!(names(main.test.df) %in% BorutaRejectedColumns)]
