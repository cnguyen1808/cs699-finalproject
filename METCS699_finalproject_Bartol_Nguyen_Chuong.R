...
train_rfModel <- function(train,test,original_test) {
  ...
  rf_pred1 <- predict(rfFit, test)
  rf_performance_pred <- compute_metrics_and_build_table(rf_pred1, test$Class)
  #Use model on main test data
  rf_pred2 <- predict(rfFit, original_test)
  rf_performance_main <- compute_metrics_and_build_table(rf_pred2, original_test$Class)
...
compute_metrics_and_build_table <- function(predicted_classes, actual_labels) {
  ...
  cm <- confusionMatrix(data = predicted_classes, 
                        reference = actual_labels)
  cm_inverted <- confusionMatrix(data = actual_labels, 
                                 reference = predicted_classes)  # Inverted confusion matrix
...
# Iterate over each balancing function
for (balance_func in balance_functions) {
  ...
  classification_models <- c("Naive Bayes", "Logistic Regression", 
  ...
  for (classification_model in classification_models) {
    ...
    # Train the model and get results for different feature selection methods
    results <- list()
    
    # Train the model on plain data
    if (classification_model == "Naive Bayes") {
      results$plain <- naive.bayes.model(training_data = balanced_training,
                                         testing_data = split.test,
                                         initaldata_test = main.test.df)
    } else if (classification_model == "Logistic Regression") {
      results$plain <- logistic.regression.model(balanced_training,
                                                 split.test,
                                                 main.test.df)
    } else if (classification_model == "Decision Tree") {
      results$plain <- DecisionTree.model(main.train.df, main.test.df)
    } else if (classification_model == "Random Forest") {
      results$plain <- train_rfModel(balanced_training, split.test, main.test.df)
    } else if (classification_model == "KNN") {
      results$plain <- train_and_select_knnModel(balanced_training, split.test, main.test.df)
    } else if (classification_model == "Neural Network") {
      results$plain <- train_nnetModel(balanced_training, split.test, main.test.df)
    }
    
    # Train the model on Boruta selected features
    if (exists("boruta.training")) {
      if (classification_model == "Naive Bayes") {
        results$boruta <- naive.bayes.model(training_data = boruta.training,
                                            testing_data = boruta.testing,
                                            initaldata_test = boruta.main.testing)
      } else if (classification_model == "Logistic Regression") {
        results$boruta <- logistic.regression.model(boruta.training,
                                                    boruta.testing,
                                                    boruta.main.testing)
      } else if (classification_model == "Decision Tree") {
        results$boruta <- DecisionTree.model(boruta.main.training, boruta.main.testing)
      } else if (classification_model == "Random Forest") {
        results$boruta <- train_rfModel(boruta.training, boruta.testing, boruta.main.testing)
      } else if (classification_model == "KNN") {
        results$boruta <- train_and_select_knnModel(boruta.training, boruta.testing, boruta.main.testing)
      } else if (classification_model == "Neural Network") {
        results$boruta <- train_nnetModel(boruta.training, boruta.testing, boruta.main.testing)
      }
    }
    
    # Train the model on Information Gain selected features
    if (exists("infogain.training")) {
      if (classification_model == "Naive Bayes") {
        results$infogain <- naive.bayes.model(training_data = infogain.training,
                                              testing_data = infogain.testing,
                                              initaldata_test = infogain.main.testing)
      } else if (classification_model == "Logistic Regression") {
        results$infogain <- logistic.regression.model(infogain.training,
                                                      infogain.testing,
                                                      infogain.main.testing)
      } else if (classification_model == "Decision Tree") {
        results$infogain <- DecisionTree.model(infogain.main.training, infogain.main.testing)
      } else if (classification_model == "Random Forest") {
        results$infogain <- train_rfModel(infogain.training, infogain.testing, infogain.main.testing)
      } else if (classification_model == "KNN") {
        results$infogain <- train_and_select_knnModel(infogain.training, infogain.testing, infogain.main.testing)
      } else if (classification_model == "Neural Network") {
        results$infogain <- train_nnetModel(infogain.training, infogain.testing, infogain.main.testing)
      }
    }
    
    # Train the model on PCA transformed features
    if (exists("pca.training")) {
      if (classification_model == "Naive Bayes") {
        results$pca <- naive.bayes.model(training_data = pca.training,
                                         testing_data = pca.testing,
                                         initaldata_test = pca.main.testing)
      } else if (classification_model == "Logistic Regression") {
        results$pca <- logistic.regression.model(pca.training,
                                                 pca.testing,
                                                 pca.main.testing)
      } else if (classification_model == "Decision Tree") {
        results$pca <- DecisionTree.model(pca.main.training, pca.main.testing)
      } else if (classification_model == "Random Forest") {
        results$pca <- train_rfModel(pca.training, pca.testing, pca.main.testing)
      } else if (classification_model == "KNN") {
        results$pca <- train_and_select_knnModel(pca.training, pca.testing, pca.main.testing)
      } else if (classification_model == "Neural Network") {
        results$pca <- train_nnetModel(pca.training, pca.testing, pca.main.testing)
      }
    }
    
    # Flatten the results and add to the list
    for (feature_selection in c("plain", "boruta", "infogain", "pca")) {
      if (exists(paste(feature_selection, ".results", sep = ""))) {
        result <- get(paste(feature_selection, ".results", sep = ""))[[balance_func]][[feature_selection]]
        result$Balance_Method <- balance_func
...
write.csv(Project_Data_Cleansed2, "preprocessed_data.csv", row.names = FALSE)

# Export the initial training and the test datasets to CSV files
write.csv(main.train.df, "initial_train.csv", row.names = FALSE)
write.csv(main.test.df, "initial_test.csv", row.names = FALSE)
